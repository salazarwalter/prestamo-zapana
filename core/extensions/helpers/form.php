...
class Form
{
    ...
    public static function selectCripto($field, $data, $attrs = '', $value = null, $blank = '', $itemId = 'id', $show = '')
    {
        $attrs = Tag::getAttrs($attrs);
        // Obtiene name, id y value (solo para autoload) para el campo y los carga en el scope
        list($id, $name, $value) = self::getFieldData($field, $value);
    ...
    public static function dbSelectCripto($field, $show = null, $data = null, $blank = 'Seleccione', $attrs = '', $value = null)
    {
        $model = ($data === null) ? substr($field, strpos($field, '.') + 1, -3) : $data[0];
        $model = Util::camelcase($model);
        $model_asoc = new $model();
        ...
        return self::selectCripto($field, $data, $attrs, $value, $blank, $pk, $show);
    }
